...
const edgeTypeStatsMap = computed(() => {
  const map = {}
  if (stats.value.edgeTypes) {
    stats.value.edgeTypes.forEach(item => {
      map[item.itemType] = item.itemCount
    })
  }
  return map
...
const renderGraph = (graphData) => {
  ...
  const option = {
    title: {
      text: currentCenterName.value ? `知识图谱 - ${currentCenterName.value}` : '知识图谱管理',
      left: 'center',
...
const handleNodeClick = async (nodeData) => {
  ...
  try {
    ...
    const related = await knowledgeApi.getRelatedNodes(Number(nodeId))
    relatedNodes.value = related || []
  } catch (error) {
    nodeDetail.value = { name: nodeData.name, nodeType: nodeData.nodeType, id: nodeId }
    nodeDetailVisible.value = true
  }
}
  ...
  try {
    ...
    if (res) {
      ...
      if (res.interpretation) {
        ElMessage.success({ message: res.interpretation, duration: 5000, showClose: true })
      }
    }
  } catch (error) {
    ...
    <div class="main-content">
      <div class="sidebar">
        <el-card class="sidebar-card">
          ...
          <div class="nav-list">
            <div :class="['nav-item', { active: activeNav === 'core' }]" @click="loadCoreNodeGraph">
              <span class="nav-icon">🏠</span> 核心疾病图谱
            </div>
            <div :class="['nav-item', { active: activeNav === 'DISEASE' }]" @click="loadTypeGraph('DISEASE')">
              <span class="nav-icon">🦠</span> 疾病关系图谱
            </div>
            <div :class="['nav-item', { active: activeNav === 'SYMPTOM' }]" @click="loadTypeGraph('SYMPTOM')">
              <span class="nav-icon">🤒</span> 症状关联图谱
            </div>
            <div :class="['nav-item', { active: activeNav === 'HERBAL' }]" @click="loadTypeGraph('HERBAL')">
              <span class="nav-icon">🌿</span> 中药方剂图谱
            </div>
            <div :class="['nav-item', { active: activeNav === 'PRESCRIPTION' }]" @click="loadTypeGraph('PRESCRIPTION')">
              <span class="nav-icon">💊</span> 方剂组成图谱
            </div>
            <div :class="['nav-item', { active: activeNav === 'FOOD_THERAPY' }]" @click="loadTypeGraph('FOOD_THERAPY')">
              <span class="nav-icon">🍲</span> 食疗养生图谱
            </div>
            <div :class="['nav-item', { active: activeNav === 'THERAPY' }]" @click="loadTypeGraph('THERAPY')">
        ...
        <el-card class="sidebar-card stats-card">
          ...
          <div class="stat-sub-title">节点类型分布</div>
          <div v-for="(count, type) in typeStatsMap" :key="type" class="stat-item">
            <span class="stat-label" :style="{ color: nodeTypeConfig[type]?.color }">
              {{ nodeTypeConfig[type]?.icon }} {{ nodeTypeConfig[type]?.label }}
            </span>
    ...
    <el-dialog v-model="nodeDetailVisible" title="节点详情" width="600px">
      <div v-if="nodeDetail" class="node-detail">
        ...
        <el-descriptions :column="1" border>
          ...
          <el-descriptions-item v-if="nodeDetail.isCoreNode !== undefined" label="核心节点">
            <el-tag :type="nodeDetail.isCoreNode === 1 ? 'success' : 'info'" size="small">
              {{ nodeDetail.isCoreNode === 1 ? '是' : '否' }}
            </el-tag>
        ...
        <div v-if="relatedNodes.length > 0" class="related-section">
          ...
          <div class="related-list">
            <el-tag
              v-for="rn in relatedNodes.slice(0, 20)"
              :key="rn.id"
              :color="nodeTypeConfig[rn.nodeType]?.color"
              effect="dark"
              size="small"
              style="margin: 2px; cursor: pointer"
              @click="nodeDetailVisible = false; loadGraphByCenterId(rn.id)"
            >
              {{ nodeTypeConfig[rn.nodeType]?.icon }} {{ rn.name }}
            </el-tag>
          </div>
        </div>
      </div>
