...
const switchAgent = (agentId) => {
  if (aiService.setPrimaryAgent(agentId)) {
    currentAgent.value = aiService.getCurrentAgent()
    ElMessage.success(`已切换到 ${currentAgent.value.config.name}`)
...
const resetAgentStatus = (agentId) => {
  aiService.resetAgentStatus(agentId)
  agentList.value = aiService.getAllAgentsStatus()
}
</script>
        ...
        <div class="agent-list">
          ...
          >
            ...
            <div class="agent-status">
              <span v-if="!agent.available" class="offline-tag" @click.stop="resetAgentStatus(agent.id)">离线-点击重试</span>
...
.chat-header {
  ...
  .header-right {
    display: flex; align-items: center; gap: 10px;
    .online-status {
      display: flex; align-items: center; gap: 6px; font-size: 12px; color: #48bb78;
...
.agent-item {
  ...
  .offline-tag { font-size: 11px; color: #ff4d4f; cursor: pointer; text-decoration: underline; }
}
