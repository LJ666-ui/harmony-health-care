...
public class ParkingController {
     ...
     * @return 记录结果
     */
    @Operation(summary = "记录停车位置", description = "用户停车后记录车位信息")
    @PostMapping("/record")
     ...
     * @return 停车信息
     */
    @Operation(summary = "获取当前停车信息", description = "查询用户当前的停车记录")
    @GetMapping("/current/{userId}")
    public Result<?> getCurrentParking(@Parameter(description = "用户ID") @PathVariable Long userId) {
        try {
     ...
     * @return 导航信息
     */
    @Operation(summary = "寻车导航", description = "获取停车位置导航信息")
    @GetMapping("/find-car/{userId}")
    public Result<?> findCar(@Parameter(description = "用户ID") @PathVariable Long userId) {
        try {
            ParkingRecord record = parkingRecordService.getCurrentParking(userId);
            if (record == null) {
                return Result.error("暂无停车记录");
     ...
     * @return 结算结果
     */
    @Operation(summary = "结算停车费用", description = "离开时结算停车费用")
    @PostMapping("/settle/{recordId}")
    public Result<?> settleParking(@Parameter(description = "记录ID") @PathVariable Long recordId) {
        try {
     ...
     * @return 支付结果
     */
    @Operation(summary = "支付停车费", description = "支付停车费用")
    @PostMapping("/pay/{recordId}")
    public Result<?> payParking(@Parameter(description = "记录ID") @PathVariable Long recordId) {
        try {
     ...
     * @return 历史记录列表
     */
    @Operation(summary = "获取停车历史", description = "查询用户的停车历史记录")
    @GetMapping("/history/{userId}")
    public Result<?> getParkingHistory(@Parameter(description = "用户ID") @PathVariable Long userId) {
        try {
     ...
     * @return 预估费用
     */
    @Operation(summary = "计算预估费用", description = "根据停车时间计算预估费用")
    @GetMapping("/calculate-fee")
    public Result<?> calculateFee(@Parameter(description = "停车时间戳") @RequestParam Long parkTime) {
        try {
     ...
     * @return 停车场信息
     */
    @Operation(summary = "获取停车场信息", description = "查询停车场详细信息")
    @GetMapping("/lot/{parkingLotId}")
    public Result<?> getParkingLotInfo(@Parameter(description = "停车场ID") @PathVariable Long parkingLotId) {
        try {
            // 模拟停车场信息
